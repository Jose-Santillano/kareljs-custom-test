...
var WorldRender = function (context, worldHeight, worldWidth) {
  ...
  this.paint = function (world, mundo_ancho, mundo_alto, options) {
    ...
    function dibuja_karel(context, world, origen) {
      ...
      context.fillStyle = '#2A7F62';
      context.beginPath();
      if (world.orientation == 0) {
        // oeste
        var karelWest = document.getElementById('k-w');
        context.drawImage(karelWest, origen.x, origen.y, 30, 30);
        context.moveTo(origen.x, origen.y + 15);
       
      } else if (world.orientation == 1) {
        // norte
        var karelNorth = document.getElementById('k-n');
        context.drawImage(karelNorth, origen.x, origen.y, 30, 30);
        context.moveTo(origen.x, origen.y + 15);

      } else if (world.orientation == 2) {
        // este
        var karelEast = document.getElementById('k-e');
        context.drawImage(karelEast, origen.x, origen.y, 30, 30);
        context.moveTo(origen.x + 2, origen.y + 8);

      } else if (world.orientation == 3) {
        // sur
        var karelSouth = document.getElementById('k-s');
        context.drawImage(karelSouth, origen.x, origen.y, 30, 30);
